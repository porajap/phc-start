<?php
  ...
  date_default_timezone_set("Asia/Bangkok");
	...
	function SearchData($conn,$DATA){
		...
		$Sql = "SELECT DocDate,DocNo,DueDate,IsSave,IsFinish,IsCancel,IsSend,DATE_FORMAT(DocDate,'%Y-%m-%d') AS DocDate,customer.Cus_Code,'-' AS Modify_Date ";
		$Sql .= "FROM bring ";
		$Sql .= "INNER JOIN customer ON bring.Cus_Code = customer.Cus_Code ";
		$Sql .= "WHERE Area_Code = '$Area'  AND IsCancel = 0 ";
		$Sql .= "AND ( DocNo LIKE '%$Search%' OR bring.Cus_Code LIKE '%$Search%' OR customer.FName  LIKE '%$Search%' )";
		if($lenSearch > 0)
