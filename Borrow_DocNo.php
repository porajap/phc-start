...
if($_REQUEST["Cus_Code"] != "" ){
	...
	if($DocNo == "" || $DocNo == null ){
			$Sql1 = "SELECT COALESCE(MAX(CONVERT(SUBSTRING(DocNo,-4),UNSIGNED INTEGER)),0)+1 AS DocNo FROM borrow ORDER BY DocNo DESC LIMIT 1";
			...
			if($DocNo != ""){
				...
				$DocNo = "A" . substr(date('Ym'),2,6) . "-" . createDigit($DocNo);
				
			}
	}

			$Cn = 0;
			$Sql2 = "SELECT DocNo,IsFinish,IsSave,IsCancel FROM borrow WHERE DocNo = '$DocNo' AND borrow.IsCancel = 0";
			...
			if($Cn == 0) {
					$Sql3 = "INSERT borrow (DocNo,DocDate,Cus_Code,Area_Code) ";
					$Sql3 .= " VALUES ";
			...
			$Sql4 .= "FROM borrow ";
			$Sql4 .= "INNER JOIN customer ON borrow.Cus_Code = customer.Cus_Code ";
			$Sql4 .= "WHERE DocNo = '$DocNo' AND borrow.IsCancel = 0";
			...
			if($_SESSION["ItemCode"] != "" && $_SESSION["Price"] != ""){
				...
				$Sql5 = "INSERT borrow_detail (DocNo,Item_Code,Qty,Price,Total,Detail) ";
				$Sql5 .= " VALUES ";
			...
			$Sql6 = "SELECT borrow_detail.Id,item.Item_Code,item.NameTH,borrow_detail.Qty,borrow_detail.Price,borrow_detail.Total ";
			$Sql6 .= "FROM item ";
			$Sql6 .= "INNER JOIN borrow_detail ON borrow_detail.Item_Code = item.Item_Code ";
...
<body>
	<div class="topnav">
        <a href="Borrow.php"><img src="img/left-blue-arrow.png" width="30px" height="30px" /></a>
					...
					<div class="row">
						<div class="col-12">
							<div class="list-group">
								...
								<a class="list-group-item list-group-item-action" href="BorrowPriceSale.php">เลือกรายการ</a>
							</div>
						</div>
					</div>
              	<?php } ?>
 ...
 <?php 	for($j=0;$j<$i;$j++){ ?>               
                  <tr>
                    <th>
                    <?php if($IsFinsh == 0){?>
                    <a href="BorrowDelItem.php?DocNo=<?php echo $DocNo ?>&Id=<?php echo $xId[$j] ?>" ><img src="img/del.png" width="16" height="16" /></a>
					<?php } ?>
					<?php echo $j+1 ?></th>
...
<?php } ?>
                ...
                <tr>
                    ...
                    <form action="BorrowDocFinish.php" method="post">
	...
	?>
		<form action="BorrowCancelBill.php" method="post">
