...
require 'connect.php';
	...
	if( ($_REQUEST["DocNo"] != "") ){
		...
		header('location:Quotation_DocNo.php?add=1&sel=1');
	}
	$Area = $_SESSION["Area"];
	...
	$Sql .= "FROM saleorder_sale ";
	//$Sql .= "WHERE saleorder_sale.DocDate BETWEEN '$sDate1' AND '$eDate1' ";
	$Sql .= "WHERE saleorder_sale.Area_Code = '$Area'  AND saleorder_sale.IsCancel = 0 ";
	...
	while ($Result = mysqli_fetch_assoc($meQuery))
	{
		...
		$Item[$i]	=  $Result["DocNo"] . " : " . $Result["DocDate"] . " : " . getCus($Result["Cus_Code"]);
		if($DueDate != "") {
...
<body>
		<div data-demo-html="true">
			<div data-role="header">
            	<a href="Quotation.php" class="ui-btn ui-corner-all ui-btn-inline ui-mini footer-button-left ui-btn-icon-left ui-icon-carat-l">กลับ</a>
...
<?php 	for($j=0;$j<$i;$j++){ ?>
					<li>
                    <a href="Bill04.php?DocNo=<?php echo $xDocNo[$j] ?>">
                            <?php      
