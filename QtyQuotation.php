...
<body>
	...
	<?php
		if($_POST["Qty"] != "" && $_POST["Price"] != ""){
			...
			$_SESSION["UnitCode"] = $_POST["UnitCode"];

			echo "<script>
			window.location.href = 'Quotation_DocNo.php?add=1&sel=0';
			</script>
			";
		}
	?>

	<form action="QtyQuotation.php" method="post">
		...
		<div class="input-group mb-3">
			...
			<select name="UnitCode" id="UnitCode" class="form-select" aria-label="Default select example">
				...
				?> 
						<option selected="selected" value="<?php echo $Result2["Unit_Code"]  ?>"><?=  getUnitName( $conn,$Result2["Unit_Code"] ); ?></option>
						<option value="<?php echo $Result2["UnitAlt_Code"]  ?>"><?=   getUnitName( $conn,$Result2["UnitAlt_Code"] );?></option>
				<?php 
					}
				?>
            </select>
