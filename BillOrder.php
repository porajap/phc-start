...
date_default_timezone_set("Asia/Bangkok");
	...
	if( ($_REQUEST["DocNo"] != "") ){
		...
		header('location:Order_DocNo.php?add=1&sel=1');
	}
	$Area = $_SESSION["Area"];
	...
	$Sql .= "FROM saleorder ";
	//$Sql .= "WHERE saleorder.DocDate BETWEEN '$sDate1' AND '$eDate1' ";
	$Sql .= "WHERE saleorder.Area_Code = '$Area'  AND saleorder.IsCancel = 0 ";
	...
	while ($Result = mysqli_fetch_assoc($meQuery))
	{
		...
		$Item[$i]	=  $Result["DocNo"] . " : " . $Result["DocDate"] . " : " . getCus($conn,$Result["Cus_Code"] ) . "<br />". ($Result["IsSave"]==1?'<div style="color:blue;">บันทึกเรียบร้อย</div>':'<div style="color:red;">รอการบันทึก</div>') ;
		if($DueDate != "") {
...
<?php 	for($j=0;$j<$i;$j++){ ?>
					<li>
                    <a href="BillOrder.php?DocNo=<?php echo $xDocNo[$j] ?>">
                            <?php      
